...
//        $write->query($query);
        ...
        if(self::CheckVisibility($orderid)){
            if($Pname = self::PurifyFileName($row['Link_FV'])) {
                if(self::InvoiceFileEntity($Pname)) {
                    $query = "insert into tim_fv (order_id,order_number,customer_id,fv_number,paid,nett_price,gross_price,payment_date,date_of_issue,link_fv)"
                            . " values (:orderid, :ordernum, :cid,:factornum, :paid, :nettprice, :grossprice, :paymentdate, :dateofissue, :linkfv)";
                    $binds = array(
                        'orderid'     => $orderid,
                        'ordernum'    => $row['Nr_oferty'],
                        'cid'         => $order->getCustomerId(),
                        'factornum'   => $row['Nr_faktury'],
                        'paid'        => $row['Zaplacona'],
                        'nettprice'   => $row['Wartosc_netto'],
                        'grossprice'  => $row['Wartosc_brutto'],
    ...
    public static function StatusUpdateEmailing($orderinfo,$demanded_status){
        ...
        if($demanded_status=='complete')
          $emailTemplate->setTemplateSubject('Twoje zamówienie zostało wysłane');
        else if($demanded_status=='processing')
          $emailTemplate->setTemplateSubject('Twoje zamówienie zostało przekazane do realizacji');  
        else
